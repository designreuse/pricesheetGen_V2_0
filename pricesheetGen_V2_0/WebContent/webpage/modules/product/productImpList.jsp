...
<body class="gray-bg">
	...
	<div class="row">
	<div class="col-sm-12">
	<form:form id="searchForm" modelAttribute="productImp" action="${ctx}/product/productImp/" method="post" class="form-inline">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<table:sortColumn id="orderBy" name="orderBy" value="${page.orderBy}" callback="sortOrRefresh();"/><!-- 支持排序 -->
	 	<div class="form-group">
			<span>商品名称：</span>
				<form:input path="pName" htmlEscape="false" maxlength="23"  class=" form-control input-sm"/>
<!-- 			<span>报价日期：</span> -->
...
<%-- 			 	<a href="#" onclick='top.openTab("${ctx}/product/productImp/addFrom","商品新增", false)' title="商品新增" class="btn btn-info btn-xs" > --%>
...
<%-- 			</shiro:hasPermission> --%>
			<shiro:hasPermission name="product:product:del">
				<table:delRow url="${ctx}/product/productImp/deleteAll" id="contentTable"></table:delRow><!-- 删除按钮 -->
			</shiro:hasPermission>
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		<thead>
			<tr>
				...
				<th  class="sort-column productName">商品类型</th>
				<th  class="sort-column productName">商品品牌</th>
				<th  class="sort-column productName">商品型号</th>
				<th  class="sort-column updateBy.name">最后更改人</th>
		...
		<c:forEach items="${page.list}" var="product">
			<tr>
				...
				<td>
    				<shiro:hasPermission name="product:productImp:add">
    					<a href="#" onclick="openDialog('商品录入', '${ctx}/product/productImp/addFrom?id=${product.id}','800px', '500px')" class="btn btn-success btn-xs" ><i class="fa fa-edit"></i>商品录入</a>
    				</shiro:hasPermission> 
    				 <shiro:hasPermission name="product:productImp:del">
						<a href="${ctx}/product/productImp/delete?id=${product.id}" onclick="return confirmx('确认要删除该商品吗？', this.href)"   class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> 删除</a>
					</shiro:hasPermission>  
				</td>
			</tr>
		</c:forEach>
