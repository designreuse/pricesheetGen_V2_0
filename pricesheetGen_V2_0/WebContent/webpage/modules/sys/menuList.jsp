...
<body class="gray-bg">
	...
	<form id="listForm" method="post">
		<table id="treeTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable" hidden="true">
			<thead><tr>
				...
				<th width="7%">可见</th>
				<th width="20%">权限标识</th><shiro:hasPermission name="sys:menu:edit">
				<th width="20%">操作</th></shiro:hasPermission>
			</tr></thead>
			<tbody><c:forEach items="${list}" var="menu">
				<tr id="${menu.id}" pId="${menu.parent.id ne '1'?menu.parent.id:'0'}">
					...
					<td title="${menu.href}">${fns:abbr(menu.href,25)}</td>
					<td style="text-align:center;">
						<shiro:hasPermission name="sys:menu:updateSort">
							<input type="hidden" name="ids" value="${menu.id}" />
							<input name="sorts" type="number" value="${menu.sort}" class="form-control" style="width:100%;margin:0;padding:0;text-align:left;">
						</shiro:hasPermission><shiro:lacksPermission name="sys:menu:updateSort">
							${menu.sort}
						</shiro:lacksPermission>
					</td>
					<td>${menu.isShow eq '1'?'显示':'隐藏'}</td>
					<td title="${menu.permission}">${fns:abbr(menu.permission,25)}</td>
					<td nowrap>
